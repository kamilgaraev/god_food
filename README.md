...
Отдельный плагин `theobroma-photo-showcases` управляет фотоблоками на Главной и странице «Корпоративные подарки». После входа откройте верхний пункт **Фотоподборки**: каждая точка показа настраивается в своей вкладке.

Для блока можно изменить надзаголовок, заголовок и описание, временно выключить публикацию, выбрать или загрузить до восьми изображений через штатную медиатеку WordPress. Порядок меняется перетаскиванием либо кнопками со стрелками. Для каждого изображения рекомендуется заполнить alt-текст; короткая видимая подпись необязательна. Удаление карточки из подборки не удаляет сам файл из медиатеки.

До первого сохранения настроек плагин использует изображения опубликованных товаров как стартовую подборку. После первого сохранения состав полностью определяется экраном **Фотоподборки**; пустая или выключенная подборка не выводит на странице пустой контейнер.
