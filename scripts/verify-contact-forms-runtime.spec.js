...
const requiredTargets = [
  '/var/www/html/wp-content/plugins/theobroma-contact-forms',
  '/var/www/html/wp-content/mu-plugins',
];
const missing = requiredTargets.filter((target) => !targets.has(target));

if (missing.length > 0) {
  process.stderr.write(`Missing WordPress runtime mounts: ${missing.join(', ')}\n`);
  process.exit(1);
}

process.stdout.write('Contact forms plugin and MU loader are mounted into WordPress.\n');
