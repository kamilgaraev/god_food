...
if (is_page('Пробники шоколада')) {
    get_template_part('template-parts/pages/chocolate-samples');
    get_footer();
    return;
}
...
<?php while (have_posts()) { the_post(); if (function_exists('is_cart') && is_account_page()) { echo '<h1 class="account-page-title">ЛИЧНЫЙ КАБИНЕТ</h1>'; } elseif (function_exists('is_cart') && (is_cart() || is_checkout())) { $accessible_title = is_cart() ? 'Корзина' : 'Оформление заказа'; echo '<h1 class="screen-reader-text">' . esc_html($accessible_title) . '</h1>'; } else { echo '<h1 class="page-title">' . esc_html(get_the_title()) . '</h1>'; } the_content(); } ?>
</div></main>
<?php get_footer();
