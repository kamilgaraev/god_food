...
$default_image_id = $default_product instanceof WC_Product
    ...
    : 0;
$default_image_url = !empty($default_profile['image_url']) ? $default_profile['image_url'] : ($default_image_id ? (string) wp_get_attachment_image_url($default_image_id, 'large') : '');
?>
<main<?php echo is_front_page() ? ' id="theobroma-main" tabindex="-1"' : ''; ?>>
                ...
                <?php if ($cacao_options) : ?>
                    <div class="home-cacao__tabs" role="tablist" aria-label="Процент какао">
                        <?php foreach ($cacao_options as $percentage => $option) : ?>
                            ...
                            $image_url = !empty($profile['image_url']) ? $profile['image_url'] : ($image_id ? (string) wp_get_attachment_image_url($image_id, 'large') : '');
                            ...
                            <button
                                ...
                                data-fact="<?php echo esc_attr(($profile['fact'] ?? '') !== '' ? $profile['fact'] : wp_strip_all_tags($product->get_short_description())); ?>"
            ...
            <?php if ($default_product instanceof WC_Product && is_array($default_group)) : ?>
                <div class="home-cacao__panel" id="home-cacao-panel" role="tabpanel" aria-labelledby="home-cacao-tab-<?php echo esc_attr((string) $default_percentage); ?>" aria-live="polite" data-cacao-panel>
                    ...
                    <div class="home-cacao__copy">
                        ...
                        <p class="home-cacao__fact" data-cacao-fact><?php echo esc_html(($default_profile['fact'] ?? '') !== '' ? $default_profile['fact'] : wp_strip_all_tags($default_product->get_short_description())); ?></p>
    ...
    <section class="home-composition" aria-labelledby="home-composition-title">
        <div class="home-composition__shell">
            <div class="home-composition__intro"><h2 id="home-composition-title">Читать этикетку приятно</h2><p>Какао-бобы, масло какао, натуральный подсластитель и экстракт ванили</p></div>
            <dl>
                ...
                <div><dt>4</dt><dd>ингредиента</dd></div>
            </dl>
        </div>
    </section>
