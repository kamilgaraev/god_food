...
$gallery = array(
    array('url' => $asset_base . 'gallery-chocolate.webp', 'alt' => 'Кусочки пористого шоколада Theobroma'),
    array('url' => $asset_base . 'gallery-packaging.webp', 'alt' => 'Подарочная коллекция Theobroma'),
    array('url' => $asset_base . 'personalization.webp', 'alt' => 'Открытка и оформление подарка'),
    array('url' => $asset_base . 'discovery.webp', 'alt' => 'Детали набора «Знакомство»'),
);
$gallery_title = '';
...
if (is_array($saved_showcases) && class_exists('Theobroma\\PhotoShowcases\\Settings')) {
    ...
    foreach ($showcase['images'] as $row) {
        $url = wp_get_attachment_image_url($row['attachment_id'], 'large');
        if ($url) {
...
<main class="corporate-gifts-page corporate-redesign" id="theobroma-main">
    <section class="cg-hero" aria-labelledby="cg-title">
        <img class="cg-hero-image" src="<?php echo esc_url($asset_base . 'hero.webp'); ?>" width="1440" height="810" fetchpriority="high" alt="Подарочные коробки шоколада Theobroma">
    ...
    <div class="cg-ribbon"><span>Без белого сахара</span><span>Доставка по всей России</span><span>Только натуральные ингредиенты</span><span>Чистый состав</span></div>
        ...
        <div class="cg-gift-grid">
            <?php foreach ($gifts as $index => $gift) : ?>
                <article class="cg-gift" data-cg-gift>
                    <a class="cg-gift-image" href="#corporate-request" data-cg-open="<?php echo esc_attr((string) $index); ?>" aria-label="Подробнее о наборе «<?php echo esc_attr($gift['name']); ?>»"><img src="<?php echo esc_url($asset_base . $gift['image'] . '.webp'); ?>" alt="Подарочный набор «<?php echo esc_attr($gift['name']); ?>»" width="720" height="430" loading="lazy" decoding="async"></a>
    ...
    <section class="cg-details cg-shell" aria-labelledby="cg-details-title">
        <div class="cg-details-intro"><h2 id="cg-details-title">Эстетика и <em>детали</em></h2><p>Мы делаем премиальное оформление со вкусом и чувством стиля.</p><img src="<?php echo esc_url($asset_base . 'personalization.webp'); ?>" width="532" height="444" loading="lazy" decoding="async" alt="Фирменная открытка на подарочной коробке"></div>
