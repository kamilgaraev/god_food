...
final class OrderWriter {
 ...
 private function order(\XMLWriter $x,OrderExportData $o):void {
  ...
  if($o->shippingName!==''||$o->shippingTotal!=='0.00'){$x->startElement('Товар');$this->e($x,'Ид','ORDER_DELIVERY');$this->e($x,'Наименование',$o->shippingName);$this->e($x,'Количество','1');$this->e($x,'ЦенаЗаЕдиницу',$o->shippingTotal);$this->e($x,'Сумма',$o->shippingTotal);$x->endElement();}foreach($o->fees as $fee){$x->startElement('Товар');$this->e($x,'Ид','ORDER_FEE_'.$fee->id);$this->e($x,'Наименование',$fee->name);$this->e($x,'Количество','1');$this->e($x,'ЦенаЗаЕдиницу',$fee->total);$this->e($x,'Сумма',$fee->total);$x->endElement();}$x->endElement();
